...
export default function SkillsSection() {
  ...
  const tools = [
    ...
    { icon: BarChart3, name: "Kendo", color: "text-success" },
    { icon: Rocket, name: "PM2", color: "text-purple-600" },
    { icon: CreditCard, name: "Razorpay", color: "text-blue-600" },
  ...
  const SkillBar = ({ skill }: { skill: { name: string; level: number } }) => (
    <div className="skill-item" data-testid={`skill-${skill.name.toLowerCase().replace(/[^\w]/g, '-')}`}>
      <div className="flex justify-between mb-2">
        <span className="font-semibold text-secondary">{skill.name}</span>
    ...
    <section className="py-20 bg-white" ref={sectionRef}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-16">
          <h2 className="text-4xl font-bold text-secondary mb-4" data-testid="skills-title">
            Technical Expertise
          </h2>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16">
          <div data-testid="backend-skills">
            <h3 className="text-2xl font-bold text-secondary mb-8">Backend Technologies</h3>
          ...
          <div data-testid="frontend-skills">
            <h3 className="text-2xl font-bold text-secondary mb-8">Frontend & Tools</h3>
            <div className="space-y-6">
        ...
        <div className="mt-16 grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-8" data-testid="tools-grid">
          {tools.map((tool, index) => (
            <div key={index} className="text-center group">
              <div className="bg-accent p-4 rounded-xl group-hover:bg-white group-hover:shadow-lg transition-all">
